...
describe('pointwiseBufferMult', () => {
  test('multiplies two arrays correctly', done => {
    expect(util.pointwiseBufferMult(
    [4, 5, 6],
    [0.5, 2, 2]
    )).toEqual([2, 10, 12]);
    done();
...
describe('frame', () => {
  test(
    'returns the expected number of frames for hop size < buffer size',
    () => {
  ...
  test(
    'returns the expected number of frames where buffer size isn\'t a hop size multiple',
    () => {
