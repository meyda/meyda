...
describe('mfcc', () => {
  test('should return correct mfcc value given a valid signal', done => {
    ...
    for (var index in en) {
      expect(Math.abs(en[index] - expectedValues[index])).toBeLessThanOrEqual(1e-15);
    }
  ...
  test(
    'should return only 3 correct mfcc values given a valid signal',
    done => {
      var en = mfcc({
        sampleRate:44100,
        bufferSize:512,
  ...
  test(
    'should return only 40 correct mfcc values given a valid signal',
    done => {
      var en = mfcc({
        sampleRate:44100,
        bufferSize:512,
